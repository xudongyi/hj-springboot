...
@Slf4j
@Service("airCurrentExcuter")
@Transactional
public class AirCurrentExcuter {
    @Autowired
    ...
    private void saveCurrentSingle(DataPacketBean dataPacketBean) {
        Map<String, FactorBean> factors = this.factorService.getFactors(FactorType.AIR.TYPE());
        Date dataTime = dataPacketBean.getDataTime();
        String mn = dataPacketBean.getMn();
        StringBuffer sql_field = new StringBuffer();
        StringBuffer sql_value = new StringBuffer();
        sql_field.append("INSERT INTO AIR_CURRENT_TR_" + CommonsUtil.dateFormat(dataTime, "yyMM") + "(ID,DATA_TIME,CREATE_TIME,MN,STATE");
