...
@Service("airDataExcuter")
@Transactional
@Slf4j
public class AirDataExcuter {
    @Autowired
        ...
        if (!this.dataParserService.isExistMHDData(dataPacketBean)) {
            ...
            if (monitor != null) {
                ...
                if (dataPacketBean.getCn().equals("2031")) {
                    ...
                    this.companyScheduleService.saveScheduleByDayData(dataPacketBean, monitor, FactorType.AIR.TYPE());
                }
            }
    ...
    private void checkData(DataPacketBean dataPacketBean, MonitorBean monitor) {
        ...
        while(var7.hasNext()) {
            ...
            FactorBean factor = (FactorBean)this.factorService.getFactors(FactorType.AIR.TYPE()).get(factorCode);
            if (factor != null) {
    ...
    private void saveMonthYearData(DataPacketBean dataPacketBean) {
        ...
        params.add(month);
        params.add(mn);
        List<Map<String, Object>> monthData = this.baseDao.sqlQuery("SELECT * FROM AIR_MONTH WHERE STATIC_TIME='"+CommonsUtil.dateFormat(dataTime, "yyyyMM")+"' AND MN='"+mn+"'");
        if (monthData == null || monthData.size()==0) {
            this.insertMonthYearData(dataPacketBean, "AIR_MONTH", month);
        } else {
            this.updateMonthYearData(dataPacketBean, "AIR_MONTH", monthData.get(0));
        }
        ...
        params.add(year);
        params.add(mn);
        List<Map<String, Object>> yearData = this.baseDao.sqlQuery("SELECT * FROM AIR_YEAR WHERE '"+CommonsUtil.dateFormat(dataTime, "yyyy")+"' AND MN='"+mn+"'");
        if (yearData == null || yearData.size()==0) {
            this.insertMonthYearData(dataPacketBean, "AIR_YEAR", year);
        } else {
            this.updateMonthYearData(dataPacketBean, "AIR_YEAR", yearData.get(0));
        }
    ...
    private void insertMonthYearData(DataPacketBean dataPacketBean, String tableName, Date staticTime) {
        ...
        while(var8.hasNext()) {
            ...
            if (this.updateTableFieldTask.isFieldExist(factorCode, FactorType.AIR.TYPE())) {
                DataFactorBean bean = (DataFactorBean)map.get(factorCode);
